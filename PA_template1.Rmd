...
```
The interval (based on the average across all the days in the dataset) that contains the maximum number of steps is `r agg_int[round(agg_int$steps,4)==round(max(agg_int$steps),4), ]$interval`, at `r round(agg_int[round(agg_int$steps,4)==round(max(agg_int$steps),4), ]$steps,2)` steps.
